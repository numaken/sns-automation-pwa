...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ユーザーのTwitterトークンを確認
    const tokenInfo = await getKVValue(`twitter_token:${userId}`);
    ...
    return res.status(200).json({
      connected: true,
      username: tokenInfo.twitterUsername,
      userId: tokenInfo.twitterUserId,
